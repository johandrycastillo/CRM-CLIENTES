...
export default function App(){
  ...
  const fetchData = async () => {
    try {
      ...
      // ... (Toda tu lógica de parseo, ddByName, ddByNit, reuByName, reuByNit, etc.)
      // ... asegúrate de copiar aquí todo el contenido que tenías dentro de tu función fetchData original
  ...
  if (!isAuthenticated) {
    return (
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100vh",background:"#0f172a",color:"#fff"}}>
        <h2 style={{marginBottom:20}}>Acceso CRM Deep Empire</h2>
        <form onSubmit={handleLogin} style={{display:"flex",flexDirection:"column",gap:10}}>
          <input type="password" placeholder="Contraseña" value={password} onChange={(e) => setPassword(e.target.value)} style={{padding:"10px",borderRadius:6, color: "#000"}}/>
          <button type="submit" style={{padding:"10px",background:"#3b82f6",color:"#fff",border:"none",borderRadius:6,cursor:"pointer"}}>Ingresar</button>
        </form>
      </div>
    );
  }
