...
export default function App(){
  ...
  const fetchData=async()=>{
    try{
      ...
      // Esto sobrescribe el respaldo embebido con los datos reales y actuales del Drive
      const ddRaw=parseRaw(ddT);
      const ddHi=ddRaw.findIndex(r=>r.some(c=>c.trim()==="RAZÓN SOCIAL"));
      if(ddHi>=0){
        const dH=ddRaw[ddHi];
        const gi=(label)=>dH.findIndex(h=>h.includes(label));
        const NIi=gi("NIT"),RSi=gi("RAZÓN SOCIAL"),SECi=gi("TIPO DE"),
              RLi=gi("REPRESENTANTE"),INGi=gi("INGRESOS"),TAMi=gi("TAMAÑO"),
              EMPi=gi("N° EMPLEADOS"),WEBi=gi("TIENE"),URLi=gi("URL PÁGINA"),
              MARi=gi("REGISTRO"),PRLi=dH.findIndex(h=>h.includes("JUDICIALES")&&h.includes("RL")&&!h.includes("SUPLENTE")),
              PRLSi=gi("RL SUPLENTE"),PEMPi=dH.findIndex(h=>h.includes("JUDICIALES")&&h.includes("EMPRESA")),
              DETi=gi("DETALLE"),ESTRi=gi("ESTRUCTURA\nJURÍDICA"),PATi=gi("PATRIMONIO"),
              RIESi=gi("RIESGOS"),PUNi=gi("PUNTAJE"),CLASi=gi("CLASIFICACIÓN");
        let liveCount=0;
        for(let i=ddHi+1;i<ddRaw.length;i++){
          const row=ddRaw[i];
          const razon=(row[RSi]||"").trim();
          if(razon.length<2)continue;
          const nombre=razon.toUpperCase();
          const nit=cleanNIT(row[NIi]||"");
          const clasRaw=(row[CLASi]||"").toUpperCase();
          ...
          const entry={
            ...
            sector:(row[SECi]||"").trim(),
            rep_legal:(row[RLi]||"").trim(),
            ingresos:(row[INGi]||"").replace(/[^0-9]/g,""),
            tamano:(row[TAMi]||"").trim(),
            empleados:(row[EMPi]||"").trim(),
            tiene_web:(row[WEBi]||"").trim(),
            url_web:(row[URLi]||"").trim(),
            marca:(row[MARi]||"").trim(),
            proc_rl:(row[PRLi]||"").trim(),
            proc_rl_sup:(row[PRLSi]||"").trim(),
            proc_empresa:(row[PEMPi]||"").trim(),
            detalle_proc:(row[DETi]||"").trim(),
            estructura_juridica:(row[ESTRi]||"").trim(),
            patrimonio:(row[PATi]||"").trim(),
            riesgos:(row[RIESi]||"").trim(),
            puntaje:parseFloat(row[PUNi])||0,
        ...
        console.log("DD live data loaded:",liveCount,"empresas desde Google Sheets");
      } else {
        console.warn("DD sheet: no se encontró header RAZÓN SOCIAL, usando solo datos embebidos");
      }
